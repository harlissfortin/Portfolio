...
| 20 | Cross-level sweep | Skills matrix marked process mining as Green Belt awareness, but no Green Belt week teaches it | `just-do-it` ✓ | Row corrected to Black Belt and above; regression, capability and hypothesis rows annotated with what each level actually covers |
| 21 | Cross-level sweep | Yellow Belt hours (12–14) and Workshop 1 duration (1.5 h) were never updated after the workshops were extended to 120 minutes | `just-do-it` ✓ | Reconciled to ≈ 15 hours, Workshop 1 at 2.0 h, across the blueprint and the level README |
| 22 | Consistency sweep | Two surfaces used different value sets for the same `sustained` badge flag (`unverified` vs `unreachable`) | `just-do-it` ✓ | Five values defined once in the 90-day sustainment check §5; every other page references that definition |
| 23 | Consistency sweep | The Black Belt delivery document said the reviewer does not attend tollgates; the tollgate standard said the reviewer is in the room | `just-do-it` ✓ | Standard governs: reviewer sits all five gates, reads the package two days ahead, writes the record, does not coach, scores the closed project |
| 24 | Second-SME review | Black Belt sections A, B and D contained numerical exhibits whose internals did not reconcile, and items where a second answer was defensible | `just-do-it` ✓ | 29 items corrected; contestable items fixed by adding a discriminator to the stem, not by moving the key; review notes filed in each section |
| 25 | Second-SME review | Sections C, E1, E2, F, G, H and I have not yet had the second-SME review policy §2 requires | `noted, not now` | The bank status page now says so per section rather than implying the whole bank is reviewed; scheduled before the first paid cohort |
